...
export const fetchDevices = async (page: number = 0, pageSize: number = 10) => {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/tenant/deviceInfos?pageSize=${pageSize}&page=${page}&sortProperty=createdTime&sortOrder=DESC`,
      {
        headers: {
...
const publishMqtt = async (payload: any): Promise<void> => {
  ...
  if (!brokerUrl)
    throw new Error("MQTT broker URL is not defined (MQTT_BROKER_URL).");
...
export async function submitPost(formData: FormData): Promise<void> {
  try {
    ...
    if (isLoop) {
      const loopTimeRaw = formData.get("loopTime");
      const loopTopic = String(formData.get("loopTopic"));
      const loopValueMin = Number(formData.get("loopValueMin"));
      const loopValueMax = Number(formData.get("loopValueMax"));
      const onlyInt = formData.has("onlyInt");

      let loopTime = Number(loopTimeRaw ?? 30);
      if (!Number.isFinite(loopTime)) loopTime = 30;

      for (let i = 0; i < loopTime; i++) {
        const randValue = onlyInt
          ? String(randInt(loopValueMin, loopValueMax))
          : randFloat(loopValueMin, loopValueMax).toFixed(2);
        const message = { [loopTopic]: randValue };
