...
import { useThingsboardStatus } from '../hooks/useConnections';
import EstablishThingsboardConnectionForm from './EstablishThingsboardConnectionForm';

const ThingsboardConnection = () => {
  const [showConnectionForm, setShowConnectionForm] = useState(false);
  const { status, loading, error, mutate } = useThingsboardStatus();
    ...
    >
      ...
      <CardFooter className="flex gap-3 px-6 pb-6 mt-auto">
        {!isConnected ? (
          <>
            <Button
              className="flex-1 bg-gray-900 hover:bg-gray-800 text-white font-medium py-2.5 transition-colors duration-200"
              disabled={loading}
            >
              Login
            </Button>
          ...
          <Button
            variant="outline"
            className="flex-1 font-medium py-2.5 transition-colors duration-200"
            disabled
          >
      ...
      {showConnectionForm && (
        ...
        >
          <EstablishThingsboardConnectionForm
...
export default ThingsboardConnection;
