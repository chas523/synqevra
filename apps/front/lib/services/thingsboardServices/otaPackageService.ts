...
import { OtaPackage, OtaPackagesPageResponse, CreateOtaPackageRequest } from "@/types/otaPackageTypes";
...
export class OtaPackageService {
    ...
    public static async createOtaPackage(payload: CreateOtaPackageRequest): Promise<OtaPackage> {
    ...
    public static async downloadOtaPackage(id: string): Promise<Blob> {
        const response = await proxyApi.get(`thingsboard/otaPackage/${id}/download`, {
            responseType: 'blob'
        });
