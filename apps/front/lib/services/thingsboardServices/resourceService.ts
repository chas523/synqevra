...
import { Resource, ResourceCreateRequest, ResourcesPageResponse } from "@/types/resourceTypes";
...
export class ResourceService {
    ...
    public static async createResource(resource: ResourceCreateRequest): Promise<Resource> {
        const { data } = await proxyApi.post("thingsboard/resources", resource);
        return data;
    }

    public static async deleteResource(resourceId: string, force: boolean = false): Promise<void> {
    ...
    public static async downloadResource(resourceId: string): Promise<Blob> {
        const response = await proxyApi.get(`thingsboard/resources/${resourceId}/download`, {
            responseType: 'blob'
        });
