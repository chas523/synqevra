...
export class OAuth2Service {
  ...
  public static async getDomainById(domainId: string): Promise<DomainInfo> {
    const { data } = await proxyApi.get(
      `thingsboard/oauth2/domain/info/${domainId}`,
    );
    return data;
  }
  ...
  public static async getOAuth2ConfigTemplates(): Promise<
    OAuth2ConfigTemplate[]
  > {
    const { data } = await proxyApi.get(`thingsboard/oauth2/config/template`);
    return data;
  }

  public static async saveOAuth2Client(
    payload: any,
  ...
  public static async getOAuth2ClientById(clientId: string): Promise<any> {
    const { data } = await proxyApi.get(
      `thingsboard/oauth2/client/${clientId}`,
    );
    return data;
  }

  public static async checkGoogleAuthAvailable(): Promise<{
    available: boolean;
  }> {
