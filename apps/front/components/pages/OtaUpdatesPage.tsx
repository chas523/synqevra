...
import { useOtaPackages, useManageOtaPackage } from "@/hooks/thingsboard/ota-packages/useOtaPackages";
...
function formatChecksum(algorithm: string | null, checksum: string | null): string {
    if (!algorithm || !checksum) return "";
    const shortChecksum = checksum.length > 10 ? checksum.substring(0, 10) + "..." : checksum;
...
export default function OtaUpdatesPage() {
    ...
    const { packages, totalPages, totalElements, isLoading, mutate } = useOtaPackages(
        currentPage,
        PAGE_SIZE,
        sortProperty,
        sortOrder
    ...
    const { isSaving, createOtaPackage, deleteOtaPackage, downloadOtaPackage } = useManageOtaPackage();
    ...
    const handleAdd = useCallback(async (payload: any) => {
        try {
            ...
            const errorMessage = error?.response?.data?.message || error?.message || "Failed to add OTA package";
            toast.error(errorMessage);
            throw error;
        }
    }, [createOtaPackage, mutate]);

    const handleDownload = useCallback(async (pkg: OtaPackage) => {
        try {
            if (!pkg.id?.id) {
                toast.error("Package ID not found");
                return;
            }
            await downloadOtaPackage(pkg.id.id, pkg.fileName || `${pkg.title}-${pkg.version}`);
            toast.success("Package downloaded successfully");
        } catch (error: any) {
            const errorMessage = error?.response?.data?.message || error?.message || "Failed to download package";
            toast.error(errorMessage);
        }
    }, [downloadOtaPackage]);

    const handleDelete = useCallback(async (pkg: OtaPackage) => {
        try {
            if (!pkg.id?.id) {
                toast.error("Package ID not found");
                return;
