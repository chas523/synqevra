...
import { GripVertical, Maximize2, Download, X, BarChart2, Activity, Thermometer, Droplets } from "lucide-react";
...
function getWidgetIcon(typeFullFqn: string) {
  if (typeFullFqn?.includes("time_series")) return <Activity className="h-4 w-4 text-blue-400" />;
  if (typeFullFqn?.includes("chart")) return <BarChart2 className="h-4 w-4 text-blue-400" />;
  if (typeFullFqn?.includes("temperature")) return <Thermometer className="h-4 w-4 text-orange-400" />;
  if (typeFullFqn?.includes("humidity")) return <Droplets className="h-4 w-4 text-cyan-400" />;
...
export function WidgetTile({ widget, isEditMode }: WidgetTileProps) {
  ...
  const title = config.title || widget.widgetConfig?.typeFullFqn?.replace("system.", "").replace(/_/g, " ") || "Widget";
  const datasources = config.datasources || [];

  const dataKeys: { name: string; label: string; color: string; units?: string }[] = [];
  datasources.forEach((ds: any) => {
    (ds.dataKeys || []).forEach((dk: any) => {
      dataKeys.push({ name: dk.name, label: dk.label || dk.name, color: dk.color, units: dk.units });
    });
  });
    ...
    <div className="group h-full w-full flex flex-col bg-white dark:bg-slate-900 rounded-md shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-shadow hover:shadow-md">
      ...
      <div
        className={`flex items-center justify-between px-3 py-2 border-b border-slate-100 dark:border-slate-700/80 bg-white dark:bg-slate-900 ${
          isEditMode ? "drag-handle cursor-grab active:cursor-grabbing select-none" : ""
        }`}
      >
      ...
      <div className="flex-1 relative overflow-hidden bg-white dark:bg-slate-900/70">
            ...
            {dataKeys.map((dk) => (
              <div key={dk.name} className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-slate-600 dark:text-slate-400">
                  {dk.label}
                  {dk.units && <span className="text-slate-400 ml-0.5">({dk.units})</span>}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="absolute inset-0 flex items-end px-4 pb-6 pt-10 gap-px">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex-1 flex flex-col justify-end items-stretch">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="text-xs text-slate-300 dark:text-slate-600 italic">No data to display</span>
        </div>
      </div>
    </div>
  );
