...
interface CustomerAlarmsTabContentProps {
  customerId: string;
}
...
interface CustomerAlarmsResponse {
  data: CustomerAlarm[];
  totalPages: number;
...
export function CustomerAlarmsTabContent({
  customerId,
}: CustomerAlarmsTabContentProps) {
  ...
  const { data, isLoading, mutate } = useSWR<CustomerAlarmsResponse>(
    customerId
      ? [
          "customerAlarms",
          customerId,
      ...
      : null,
    async () =>
      AssetService.getCustomerAlarms(
        customerId,
        page,
        pageSize,
        statusList,
  ...
  const columns: DataTableColumn<CustomerAlarm>[] = useMemo(
    () => [
      {
        key: "createdTime",
        header: "Created time",
  ...
  const filteredAlarms = useMemo(() => {
    ...
    return (data?.data || []).filter((alarm: CustomerAlarm) => {
      ...
      <DataTable
        ...
        emptyMessage={
          searchQuery.trim()
            ...
            : "No alarms found for this customer."
        }
