...
export const DeviceCredentialsDialog = ({
  ...
  const [serverCreds, setServerCreds] = useState<DeviceCredentials | null>(
    null,
  );
    ...
    const loadCredentials = async () => {
      ...
      try {
        const creds: DeviceCredentials =
          await DeviceService.getDeviceCredentials(device.id?.id ?? "");
  ...
  const applyCredentials = (creds: DeviceCredentials) => {
    ...
    if (creds.credentialsType === "ACCESS_TOKEN") {
      ...
      try {
        const parsed: MqttBasicValue = JSON.parse(
