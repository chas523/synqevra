...
export const CreateWidgetBundleDialog = ({ open, onOpenChange, onSuccess }: CreateWidgetBundleDialogProps) => {
    const [showImageSelector, setShowImageSelector] = useState(false);

    const { control, handleSubmit, register, setValue, watch, reset, formState: { errors } } = useForm<FormValues>({
    ...
    const onSubmit = async (values: FormValues) => {
        try {
            ...
            const errorMessage = error?.response?.data?.message || 'Failed to create widget bundle';
                ...
                <DialogContent className="sm:max-w-[600px]">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="title" placeholder="Bundle Title" {...register('title')} />
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-4 items-start">
                                <div className="h-24 w-24 border rounded-md overflow-hidden bg-muted flex items-center justify-center">
                                    {imageValue ? (
                                        <img src={imageValue} alt="Preview" className="h-full w-full object-cover" />
                                    ) : (
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    {imageValue && (
                                        <Button type="button" variant="ghost" size="sm" onClick={() => setValue('image', '')} className="text-destructive hover:text-destructive">
                                            Remove
                                        </Button>
                                    )}
                                </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea id="description" placeholder="Bundle description..." {...register('description')} />
                        </div>
