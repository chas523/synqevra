...
function AttributesTabContent({ profileId }: { profileId: string }) {
  ...
  const fetchAttributes = async () => {
    try {
      setLoading(true);
      const data = await TenantService.getTenantProfileAttributes(
        profileId,
        scope,
      );
      setAttributes(data);
  ...
  const handleAddAttribute = async (
    key: string,
    value: unknown,
    _valueType: AttributeValueType,
  ) => {
    await TenantService.saveTenantProfileAttributes(
      profileId,
      { [key]: value },
      scope,
    );
...
export function TenantProfileDetailPanel({
  ...
  const [calculatedFieldsAdvanced, setCalculatedFieldsAdvanced] =
    useState(false);
      ...
      setFormState({
        ...
        configuration: {
          ...
          maxRuleNodeExecutionsPerMessage:
            config.maxRuleNodeExecutionsPerMessage ?? 0,
          maxExecuteRateLimit: config.maxExecuteRateLimit ?? 0,

          // Calculated Fields
          maxCalculatedFieldsPerEntity:
            config.maxCalculatedFieldsPerEntity ?? 0,
          ...
          maxSingleValueArgumentSizeInKBytes:
            config.maxSingleValueArgumentSizeInKBytes ?? 0,
          ...
          transportTenantTelemetryMsgRateLimit:
            config.transportTenantTelemetryMsgRateLimit ?? "",
          transportTenantTelemetryDataPointsRateLimit:
            config.transportTenantTelemetryDataPointsRateLimit ?? "",
          transportDeviceMsgRateLimit: config.transportDeviceMsgRateLimit ?? "",
          transportDeviceTelemetryMsgRateLimit:
            config.transportDeviceTelemetryMsgRateLimit ?? "",
          transportDeviceTelemetryDataPointsRateLimit:
            config.transportDeviceTelemetryDataPointsRateLimit ?? "",
          transportGatewayMsgRateLimit:
            config.transportGatewayMsgRateLimit ?? "",
          transportGatewayTelemetryMsgRateLimit:
            config.transportGatewayTelemetryMsgRateLimit ?? "",
          transportGatewayTelemetryDataPointsRateLimit:
            config.transportGatewayTelemetryDataPointsRateLimit ?? "",
          ...
          tenantNotificationRequestsRateLimit:
            config.tenantNotificationRequestsRateLimit ?? "",
          tenantNotificationRequestsPerRuleRateLimit:
            config.tenantNotificationRequestsPerRuleRateLimit ?? "",
          type: config.type || "DEFAULT",
        },
        queueConfiguration: (tenantProfile.profileData?.queueConfiguration ||
          []) as QueueConfig[],
      });
    }
  }, [tenantProfile]);
  ...
  const hasChanges = useMemo(() => {
    ...
    const basicChanged =
      ...
      formState.isolatedTbRuleEngine !==
        (tenantProfile.isolatedTbRuleEngine || false);
  ...
  const updateField =
    <K extends keyof typeof formState>(field: K) =>
    (value: (typeof formState)[K]) => {
      setFormState((prev) => ({ ...prev, [field]: value }));
    };

  const updateConfigField =
    (field: keyof typeof formState.configuration) => (value: number) => {
      setFormState((prev) => ({
        ...prev,
        configuration: {
          ...prev.configuration,
  ...
  const updateConfigStringField =
    (field: keyof typeof formState.configuration) => (value: string) => {
      setFormState((prev) => ({
        ...prev,
        configuration: {
          ...prev.configuration,
            ...
            <div className="py-3 border-b border-slate-200 dark:border-slate-700/50">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    updateField("isolatedTbRuleEngine")(e.target.checked)
                  }
                  disabled={
                    formState.queueConfiguration.length > 0 ||
                    tenantProfile.name === "Default"
                  }
                ...
                />
                <span
                  className={`text-sm ${formState.queueConfiguration.length > 0 || tenantProfile.name === "Default" ? "text-slate-400 dark:text-slate-500" : "text-slate-700 dark:text-slate-300"}`}
                >
        ...
        <div className="pb-6 max-h-[calc(100vh-200px)] overflow-y-auto">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {/* Entities Section */}
              <ConfigSection
                title="Entities (0 - unlimited)"
                defaultExpanded={true}
              >
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Devices maximum number*"
                    value={formState.configuration.maxDevices}
                    onChange={updateConfigField("maxDevices")}
                  />
                  <EditableNumberField
                    label="Dashboards maximum number*"
                    value={formState.configuration.maxDashboards}
                    onChange={updateConfigField("maxDashboards")}
                  />
                  <EditableNumberField
                    label="Assets maximum number*"
                    value={formState.configuration.maxAssets}
                    onChange={updateConfigField("maxAssets")}
                  />
                  <EditableNumberField
                    label="Users maximum number*"
                    value={formState.configuration.maxUsers}
                    onChange={updateConfigField("maxUsers")}
                  />
                </div>
                ...
                {entitiesAdvanced && (
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableNumberField
                      label="Customers maximum number*"
                      value={formState.configuration.maxCustomers}
                      onChange={updateConfigField("maxCustomers")}
                    />
                    <EditableNumberField
                      label="Rule chains maximum number*"
                      value={formState.configuration.maxRuleChains}
                      onChange={updateConfigField("maxRuleChains")}
                    />
                    <EditableNumberField
                      label="Edges maximum number*"
                      value={formState.configuration.maxEdges}
                      onChange={updateConfigField("maxEdges")}
                    />
                  </div>
                )}
              </ConfigSection>
              ...
              <ConfigSection title="Rule Engine (0 - unlimited)">
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Rule Engine executions maximum number*"
                    value={formState.configuration.maxREExecutions}
                    onChange={updateConfigField("maxREExecutions")}
                  />
                  <EditableNumberField
                    label="Transport messages maximum number*"
                    value={formState.configuration.maxTransportMessages}
                    onChange={updateConfigField("maxTransportMessages")}
                  />
                </div>
                ...
                {ruleEngineAdvanced && (
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableNumberField
                      label="JS functions executions maximum number*"
                      value={formState.configuration.maxJSExecutions}
                      onChange={updateConfigField("maxJSExecutions")}
                    />
                    <EditableNumberField
                      label="Transport data points maximum number*"
                      value={formState.configuration.maxTransportDataPoints}
                      onChange={updateConfigField("maxTransportDataPoints")}
                    />
                    <EditableNumberField
                      label="Rule node executions per message maximum number*"
                      value={
                        formState.configuration.maxRuleNodeExecutionsPerMessage
                      }
              ...
              <ConfigSection title="Calculated fields (0 - unlimited)">
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Calculated fields per entity maximum number*"
                    value={formState.configuration.maxCalculatedFieldsPerEntity}
                    onChange={updateConfigField("maxCalculatedFieldsPerEntity")}
                  />
                  <EditableNumberField
                    label="Max data points number in rolling arguments*"
                    value={formState.configuration.maxDataPointsPerRollingArg}
                    onChange={updateConfigField("maxDataPointsPerRollingArg")}
                  />
                  <EditableNumberField
                    label="Arguments per calculated field max number*"
                    value={formState.configuration.maxArgumentsPerCF}
                    onChange={updateConfigField("maxArgumentsPerCF")}
                  />
                </div>
                ...
                <button
                  type="button"
                  onClick={() =>
                    setCalculatedFieldsAdvanced(!calculatedFieldsAdvanced)
                  }
                  className="text-xs text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 mb-2 flex items-center gap-1"
                >
                  Advanced settings
                ...
                {calculatedFieldsAdvanced && (
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableNumberField
                      label="State maximum size in KB*"
                      value={formState.configuration.maxStateSizeInKBytes}
                      onChange={updateConfigField("maxStateSizeInKBytes")}
                    />
                    <EditableNumberField
                      label="Single value argument maximum size in KB*"
                      value={
              ...
              <ConfigSection title="Time-to-live (0 - unlimited)">
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Data points storage days maximum number*"
                    value={formState.configuration.defaultStorageTtlDays}
                    onChange={updateConfigField("defaultStorageTtlDays")}
                  />
                  <EditableNumberField
                    label="Alarms TTL days*"
                    value={formState.configuration.alarmsTtlDays}
                    onChange={updateConfigField("alarmsTtlDays")}
                  />
                  <EditableNumberField
                    label="Storage TTL days by default*"
                    value={formState.configuration.maxDPStorageDays}
                    onChange={updateConfigField("maxDPStorageDays")}
                  />
                  <EditableNumberField
                    label="RPC TTL days*"
                    value={formState.configuration.rpcTtlDays}
                    onChange={updateConfigField("rpcTtlDays")}
                  />
                  <EditableNumberField
                    label="Queue stats TTL days*"
                    value={formState.configuration.queueStatsTtlDays}
                    onChange={updateConfigField("queueStatsTtlDays")}
                  />
                  <EditableNumberField
                    label="Rule Engine exceptions TTL days*"
                    value={formState.configuration.ruleEngineExceptionsTtlDays}
                    onChange={updateConfigField("ruleEngineExceptionsTtlDays")}
                  />
                </div>
              </ConfigSection>
              ...
              <ConfigSection title="Alarms and notifications (0 - unlimited)">
                <div className="mb-4">
                  <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                    <div
                      className={`w-4 h-4 rounded-full border-2 ${config?.smsEnabled ? "bg-slate-400 border-slate-400" : "border-slate-300 dark:border-slate-600"}`}
                    />
                    SMS enabled
                  </label>
                </div>
                ...
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Emails sent maximum number*"
                    value={formState.configuration.maxEmails}
                    onChange={updateConfigField("maxEmails")}
                  />
                  <EditableNumberField
                    label="Alarms created maximum number*"
                    value={formState.configuration.maxCreatedAlarms}
                    onChange={updateConfigField("maxCreatedAlarms")}
                  />
                </div>
              </ConfigSection>
              ...
              <ConfigSection title="Debug">
                <EditableNumberField
                  label="Maximum debug duration (min)"
                  value={formState.configuration.maxDebugModeDurationMinutes}
                  onChange={updateConfigField("maxDebugModeDurationMinutes")}
                />
              </ConfigSection>
              ...
              <ConfigSection title="WS (0 - unlimited)">
                <div className="grid grid-cols-2 gap-x-4">
                  <EditableNumberField
                    label="Sessions per tenant maximum number"
                    value={formState.configuration.maxWsSessionsPerTenant}
                    onChange={updateConfigField("maxWsSessionsPerTenant")}
                  />
                  <EditableNumberField
                    label="Subscriptions per tenant maximum number"
                    value={formState.configuration.maxWsSubscriptionsPerTenant}
                    onChange={updateConfigField("maxWsSubscriptionsPerTenant")}
                  />
                  <EditableNumberField
                    label="Sessions per customer maximum number"
                    value={formState.configuration.maxWsSessionsPerCustomer}
                    onChange={updateConfigField("maxWsSessionsPerCustomer")}
                  />
                  <EditableNumberField
                    label="Subscriptions per customer maximum number"
                    value={
                      formState.configuration.maxWsSubscriptionsPerCustomer
                    }
                    onChange={updateConfigField(
                      "maxWsSubscriptionsPerCustomer",
                    )}
                ...
                {wsAdvanced && (
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableNumberField
                      label="Sessions per regular user maximum number"
                      value={
                        formState.configuration.maxWsSessionsPerRegularUser
                      }
                      onChange={updateConfigField(
                    ...
                    />
                    <EditableNumberField
                      label="Subscriptions per regular user maximum number"
                      value={
                        formState.configuration.maxWsSubscriptionsPerRegularUser
                      }
                      onChange={updateConfigField(
                        "maxWsSubscriptionsPerRegularUser",
                      )}
                    />
                    <EditableNumberField
                      label="Sessions per public user maximum number"
                      value={formState.configuration.maxWsSessionsPerPublicUser}
                      onChange={updateConfigField("maxWsSessionsPerPublicUser")}
                    />
                    <EditableNumberField
                      label="Subscriptions per public user maximum number"
                      value={
                        formState.configuration.maxWsSubscriptionsPerPublicUser
                      }
                      onChange={updateConfigField(
                        "maxWsSubscriptionsPerPublicUser",
                      )}
                    />
                    <EditableNumberField
                      label="WS message queue limit per session"
                      value={formState.configuration.wsMsgQueueLimitPerSession}
                      onChange={updateConfigField("wsMsgQueueLimitPerSession")}
                    />
                  </div>
                )}
              </ConfigSection>
              ...
              <ConfigSection title="Rate limits">
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableTextField
                      label="Transport tenant messages"
                      value={
                        formState.configuration.transportTenantMsgRateLimit
                      }
                      onChange={updateConfigStringField(
                        "transportTenantMsgRateLimit",
                      )}
                    />
                    <EditableTextField
                      label="Transport device messages"
                      value={
                        formState.configuration.transportDeviceMsgRateLimit
                      }
                      onChange={updateConfigStringField(
                        "transportDeviceMsgRateLimit",
                      )}
                  ...
                  <div className="grid grid-cols-2 gap-x-4">
                    <EditableTextField
                      label="Transport tenant telemetry messages"
                      value={
                        formState.configuration
                          .transportTenantTelemetryMsgRateLimit
                      }
                    ...
                    />
                    <EditableTextField
                      label="Transport device telemetry messages"
                      value={
                        formState.configuration
                          .transportDeviceTelemetryMsgRateLimit
                      }
                  ...
                  <div className="grid grid-cols-2 gap-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 rounded border border-slate-200 dark:border-slate-700">
                        {config?.transportGatewayDeviceMsgRateLimit ||
                          "Not set"}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 rounded border border-slate-200 dark:border-slate-700">
                        {config?.transportGatewayDeviceTelemetryMsgRateLimit ||
                          "Not set"}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4 mt-4">
                      ...
                      <div className="grid grid-cols-2 gap-x-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 rounded border border-slate-200 dark:border-slate-700">
                            {config?.transportGatewayDeviceTelemetryDataPointsRateLimit ||
                              "Not set"}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-x-4">
                        <EditableTextField
                          ...
                          value={
                            formState.configuration.tenantEntityExportRateLimit
                          }
                          onChange={updateConfigStringField(
                            "tenantEntityExportRateLimit",
                          )}
                        ...
                        <EditableTextField
                          ...
                          value={
                            formState.configuration.tenantEntityImportRateLimit
                          }
                          onChange={updateConfigStringField(
                            "tenantEntityImportRateLimit",
                          )}
                      ...
                      <div className="grid grid-cols-2 gap-x-4">
                        ...
                        />
                        <EditableTextField
                          label="Notification requests per notification rule"
                          value={
                            formState.configuration
                              .tenantNotificationRequestsPerRuleRateLimit
                          }
                      ...
                      <div className="grid grid-cols-2 gap-x-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 rounded border border-slate-200 dark:border-slate-700">
                            {config?.edgeUplinkMessagesRateLimitsPerEdge ||
                              "Not set"}
                          </div>
                        </div>
                      </div>
              ...
              <ConfigSection
                title={`Queues (${formState.queueConfiguration.length})`}
              >
                <div className="mb-4">
                  <button
                    type="button"
                  ...
                  {!formState.isolatedTbRuleEngine && (
                    <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                      Enable "Use isolated ThingsBoard Rule Engine queues" to
                      configure queues.
                    </p>
                  )}
                </div>
                ...
                {formState.queueConfiguration.length === 0 ? (
                  ...
                  formState.queueConfiguration.map((queue, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="mb-6 last:mb-0 border border-slate-200 dark:border-slate-700 rounded-lg p-4"
                      >
                          ...
                          <div className="border border-slate-200 dark:border-slate-700 rounded-lg">
                            ...
                            <div className="p-4 space-y-3">
                              <div>
                                ...
                                <div className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 rounded border border-slate-200 dark:border-slate-700">
                                  {formatStrategyType(
                                    queue.submitStrategy.type,
                                  )}
                                </div>
                              </div>
                              {(queue.submitStrategy.type === "BURST" ||
                                queue.submitStrategy.type === "BATCH") && (
                                <div>
                                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">
                                    Batch size
                                  </label>
