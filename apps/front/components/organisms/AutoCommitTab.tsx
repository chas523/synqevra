...
export function AutoCommitTab() {
    const { isConfigured, isLoading: isLoadingInfo, mutate: mutateInfo } = useRepoSettingsInfo();
    const { isChecking, isSaving: isSavingRepo, checkAccess, saveSettings: saveRepoSettings } = useManageRepoSettings();
    ...
    if (!isConfigured) {
        return <NotConfiguredView onCheckAccess={checkAccess} onSave={async (payload) => { await saveRepoSettings(payload); await mutateInfo(); }} isChecking={isChecking} isSaving={isSavingRepo} />;
    }
        ...
        <div className="space-y-4">
            <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 rounded-xl p-6 shadow-sm">
                <p className="text-sm text-muted-foreground dark:text-slate-400 mb-4">
                    Repository is not configured. Please configure version control first to use auto-commit.
                </p>
            </div>
...
function AutoCommitForm() {
    ...
    useEffect(() => {
        async function load() {
            try {
                const settings = await SettingsService.getAutoCommitSettings().catch(() => null);
    ...
    const handleAddEntity = useCallback(() => {
        ...
        setEntries((prev) => [
            ...prev,
            { id: generateId(), entityType: first.value, branch: defaultBranch, options },
        ]);
    ...
    const handleDelete = useCallback(async () => {
        if (!confirm("Are you sure you want to delete all auto-commit settings?")) return;
        setIsDeleting(true);
        try {
    ...
    return (
        <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 rounded-xl p-6 shadow-sm">
            ...
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-foreground dark:text-white">Auto-commit settings</h2>
                <button
                    type="button"
            ...
            <fieldset className="border border-slate-300 dark:border-slate-600 rounded-md p-4 mb-4">
                <legend className="text-sm text-muted-foreground dark:text-slate-400 px-2">Auto-commit entities</legend>

                {entries.length === 0 ? (
                    <div className="flex items-center justify-center py-8">
                        <p className="text-sm text-muted-foreground dark:text-slate-400">No entities configured for auto-commit</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {entries.map((entry) => (
                            <EntityEntryCard
                                ...
                                onChangeBranch={(branch) => handleChangeBranch(entry.id, branch)}
    ...
    return (
        <fieldset className="border border-slate-300 dark:border-slate-600 rounded-md p-4">
            ...
            <div className="flex items-start gap-4 mb-3">
                <div className="w-40">
                    <label className="text-xs text-muted-foreground dark:text-slate-400 block mb-1">Type*</label>
                ...
                <div className="w-40">
                    <label className="text-xs text-muted-foreground dark:text-slate-400 block mb-1">Branch</label>
                    <Select value={entry.branch} onValueChange={onChangeBranch} onOpenChange={(open) => { if (open) onOpenBranchDropdown(); }}>
                        <SelectTrigger className="w-full dark:text-white border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800/50">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                {entityConfig?.exportOptions.map((opt) => (
                    <label key={opt.key} className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-sm text-foreground dark:text-white select-none">{opt.label}</span>
                    </label>
                ))}
            </div>
        </fieldset>
