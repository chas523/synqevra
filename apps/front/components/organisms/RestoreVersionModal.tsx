    ...
    if (open) {
      const defaultEntries = ENTITY_TYPE_CONFIGS.map((config) => {
        const options: Record<string, boolean> = {
          ...
          loadAttributes: config.exportOptions.some(
            (o) => o.key === "saveAttributes",
          )
            ? true
            : false,
          loadRelations: config.exportOptions.some(
            (o) => o.key === "saveRelations",
          )
            ? false
            : false,
          loadCredentials: config.exportOptions.some(
            (o) => o.key === "saveCredentials",
          )
            ? true
            : false,
          loadCalculatedFields: config.exportOptions.some(
            (o) => o.key === "saveCalculatedFields",
          )
            ? false
            : false,
        };
        ...
        if (
          ["ASSET", "DEVICE", "DEVICE_PROFILE", "ASSET_PROFILE"].includes(
            config.value,
          )
        ) {
  ...
  const handleRestore = async () => {
    ...
    entries.forEach((entry) => {
      entityTypesPayload[entry.entityType] = {
        ...
        findExistingEntityByName:
          entry.options.findExistingEntityByName ?? true,
      };
    });
    ...
    try {
      ...
      setResult({
        ...
        modified: 0,
        removed: 0,
        error: e.message || "Failed to start restoration",
      });
      setIsRestoring(false);
    }
  };
        ...
        if (response.done) {
          ...
          if (response.error) {
            setResult({
              ...
              removed: 0,
              error:
                response.error.type || "An error occurred during restoration",
            });
          } else {
            const added =
              response.result?.reduce(
                (acc: number, item: any) => acc + (item.created || 0),
                0,
              ) || 0;
            const modified =
              response.result?.reduce(
                (acc: number, item: any) => acc + (item.updated || 0),
                0,
              ) || 0;
            const removed =
              response.result?.reduce(
                (acc: number, item: any) => acc + (item.deleted || 0),
                0,
              ) || 0;
            ...
            return (
              ...
              >
                <div className="relative">
                  ...
                  />
                  <div
                    className={`w-4 h-4 border-2 ${isChecked ? "border-orange-500 dark:border-orange-400 bg-orange-500 dark:bg-orange-400" : "border-slate-300 dark:border-slate-600"} rounded-sm flex items-center justify-center transition-colors ${disabled ? "opacity-50" : ""}`}
                  >
                    {isChecked && (
                      <svg
