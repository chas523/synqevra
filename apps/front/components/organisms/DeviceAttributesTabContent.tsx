...
export function DeviceAttributesTabContent({
  ...
  const {
    ...
    deviceId ? ["deviceAttributes", deviceId, scope] : null,
    async () => {
      if (scope === "SERVER_SCOPE") {
        return DeviceService.fetchDeviceServerAttributes(deviceId);
      }
      ...
      return DeviceService.fetchDeviceClientAttributes(deviceId);
    },
  );
  ...
  const handleAddAttribute = async () => {
    ...
    try {
      if (scope === "SERVER_SCOPE") {
        await DeviceService.updateDeviceServerAttributes(deviceId, {
          [key]: parsedValue,
        });
      } else {
        await DeviceService.updateDeviceSharedAttributes(deviceId, {
          [key]: parsedValue,
        });
      }
      ...
      toast.error(
        error?.response?.data?.message || "Failed to add device attribute",
      );
    } finally {
      setIsSaving(false);
    }
      ...
      <DataTable
        ...
        emptyMessage={
          searchQuery.trim()
            ...
            : "No attributes found for this device."
        }
        ...
        <DialogContent className="sm:max-w-130">
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-[160px_1fr]">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="device-attribute-value">Value*</Label>
                <Input
            ...
            <div className="space-y-1.5">
              <Label htmlFor="device-attribute-key">Key*</Label>
              <Input
                id="device-attribute-key"
                value={attributeKey}
