...
export function AssetDetailsTabContent({
  assetId,
}: AssetDetailsTabContentProps) {
  const {
    data: asset,
    isLoading,
    mutate,
  } = useSWR(assetId ? ["assetDetails", assetId] : null, async () =>
    AssetService.fetchAsset(assetId),
  );
  ...
  const [originalForm, setOriginalForm] =
    useState<AssetDetailsFormState | null>(null);
              ...
              {isEditing ? (
                <select
                  ...
                  onChange={(event) =>
                    setForm((prev) =>
                      prev
                        ? { ...prev, assetProfileId: event.target.value }
                        : prev,
                    )
                  }
                  className="w-full bg-transparent outline-none"
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="space-y-1">
              ...
              <div className="ml-1 text-sm text-slate-700">
                {asset.createdTime
                  ? new Date(asset.createdTime).toLocaleString()
                  : "-"}
              </div>
            </div>
