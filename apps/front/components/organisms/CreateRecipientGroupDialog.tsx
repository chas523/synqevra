...
export function CreateRecipientGroupDialog({
  ...
  const [userFilter, setUserFilter] = useState<UserFilterType>("ALL_USERS");
  const [selectionMode, setSelectionMode] = useState<SelectionMode>("tenant");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
          ...
          usersFilter: {
            ...
            ...(userFilter === "TENANT_ADMINISTRATORS" &&
              ...
              selectedProfiles.length > 0 && {
                tenantProfilesIds: selectedProfiles.map((p) => p.id.id),
              }),
          },
      ...
      const response = await fetch("/api/thingsboard/notification/target", {
        method: "POST",
        headers: {
      ...
      <DialogContent className="sm:max-w-150 overflow-visible">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={userFilter}
              onValueChange={(value) => setUserFilter(value as UserFilterType)}
              options={[
