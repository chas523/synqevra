...
export function CreateVersionModal({ open, onOpenChange, branches, onSuccess }: CreateVersionModalProps) {
    ...
    const usedEntityTypes = useMemo(() => new Set(entries.map((e) => e.entityType)), [entries]);
    const availableEntityTypes = useMemo(() => ENTITY_TYPE_CONFIGS.filter((c) => !usedEntityTypes.has(c.value)), [usedEntityTypes]);
    ...
    const handleAddEntity = useCallback(() => {
        ...
        setEntries((prev) => [
            ...prev,
            { id: generateId(), entityType: first.value, syncStrategy: "Default", allEntities: true, options, entityIds: [] },
        ]);
    ...
    const handleChangeSyncStrategy = useCallback((id: string, syncStrategy: string) => {
        setEntries((prev) => prev.map((entry) => (entry.id === id ? { ...entry, syncStrategy } : entry)));
    }, []);

    const handleToggleAllEntities = useCallback((id: string, allEntities: boolean) => {
        setEntries((prev) => prev.map((entry) => (entry.id === id ? { ...entry, allEntities, entityIds: allEntities ? [] : entry.entityIds } : entry)));
    }, []);
    ...
    const handleSetEntityIds = useCallback((id: string, entityIds: string[]) => {
        setEntries((prev) => prev.map((entry) => (entry.id === id ? { ...entry, entityIds } : entry)));
    }, []);
    ...
    const handleCreate = async () => {
        ...
        for (const entry of entries) {
            entityTypesObj[entry.entityType] = {
                syncStrategy: entry.syncStrategy === "Default" ? null : entry.syncStrategy.toUpperCase(),
                ...entry.options,
        ...
        return (
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="sm:max-w-[400px] p-6 text-center">
                    <DialogHeader>
                        <DialogTitle className="dark:text-white text-xl mx-auto mb-4">Version Details</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4 py-4 text-foreground dark:text-white">
                        <p className="font-semibold text-lg">{result.added === 0 && result.modified === 0 && result.removed === 0 ? "No entities added." : "Version created successfully."}</p>
        ...
        <Dialog open={open} onOpenChange={(val) => !isCreating && !isPolling && onOpenChange(val)}>
            <DialogContent className="max-w-[90vw] md:max-w-6xl max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0">
                <DialogHeader className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                    <DialogTitle className="text-xl dark:text-white">Create entities version</DialogTitle>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
                    ...
                    <div className="flex gap-4">
                        <div className="flex-1 space-y-2">
                            <label className="text-sm font-medium text-muted-foreground dark:text-slate-400">Branch*</label>
                        ...
                        <div className="flex-1 space-y-2">
                            <label className="text-sm font-medium text-muted-foreground dark:text-slate-400">Version name*</label>
                            <Input
                                value={versionName}
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-muted-foreground dark:text-slate-400">Default sync strategy*</label>
                        <Select value={defaultSyncStrategy} onValueChange={setDefaultSyncStrategy} disabled={isCreating || isPolling}>
                            <SelectTrigger className="w-full bg-slate-50 dark:bg-slate-800/50 h-10 dark:text-white">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <p className="text-xs text-muted-foreground pt-1 dark:text-slate-400">
                            Creates or updates selected entities in the repository. All other repository entities are <strong>not modified</strong>.
                        </p>
                    </div>

                    {/* Entities fieldset */}
                    <fieldset className="border border-slate-200 dark:border-slate-800 rounded-lg p-5">
                        <legend className="text-sm font-medium text-foreground dark:text-slate-300 px-2">Entities to export</legend>
                        ...
                        <div className="space-y-4 mb-4">
                            {entries.map(entry => (
                                <EntityEntryCard
                                    ...
                                    onChangeSyncStrategy={(strategy) => handleChangeSyncStrategy(entry.id, strategy)}
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/30 p-2 rounded border border-dashed border-slate-300 dark:border-slate-700">
                            <Button
                                ...
                                disabled={availableEntityTypes.length === 0 || isCreating || isPolling}
                ...
                <DialogFooter className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 sm:justify-end">
                    <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isCreating || isPolling}>
                        Cancel
                    </Button>
                    <Button
                        onClick={handleCreate}
                        disabled={isCreating || isPolling || !versionName.trim() || entries.some(e => !e.allEntities && e.entityIds.length === 0)}
            ...
            <div className="p-4 flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-4">
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-1.5">
                            <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Sync strategy</label>
                            <Select value={entry.syncStrategy} onValueChange={onChangeSyncStrategy} disabled={disabled}>
                                <SelectTrigger className="h-9 bg-slate-50 dark:bg-slate-800/50 dark:text-white">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        ...
                        />
                        <span className="text-sm font-medium dark:text-white">All entities</span>
                    </div>
                    ...
                    {entityConfig?.exportOptions.map((opt) => (
                        <label key={opt.key} className="flex items-center gap-2 cursor-pointer">
                            <div className="relative">
                                ...
                                />
                                <div className={`w-4 h-4 border-2 border-orange-500 dark:border-orange-400 rounded-sm flex items-center justify-center transition-colors ${disabled ? 'opacity-50' : ''} peer-checked:bg-orange-500 dark:peer-checked:bg-orange-400`}>
...
function EntityMultiSelect({
    ...
    const [selectedEntitiesCache, setSelectedEntitiesCache] = useState<Record<string, any>>({});
    ...
    const loadEntitiesForType = async (typeToLoad: string) => {
        setIsLoading(true);
        try {
            const data = await VersionControlService.getEntitiesByType(typeToLoad, 0, 100); // Fetch up to 100 for now
            if (data?.data) {
    ...
    return (
        <div className="relative">
            ...
            >
                {selectedIds.map(id => {
                    const entity = selectedEntitiesCache[id] || entities.find(e => e.id?.id === id);
                    return (
                        <span key={id} className="flex items-center gap-1 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded-full text-xs dark:text-white" title={id}>
                            {entity?.title || entity?.name || id.substring(0, 8)}
                            <button className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 ml-1" onClick={(e) => removeId(id, e)}>
                                ×
                            </button>
                        </span>
                    );
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-60 overflow-y-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded shadow-md">
                        {isLoading && <div className="p-3 text-sm text-center text-slate-500 border-b border-slate-200 dark:border-slate-700 dark:text-slate-400">Loading...</div>}
                        {entities.length > 0 && (
                            <div className="p-2 border-b border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-500 dark:text-slate-400 flex justify-between items-center">
                                <span>List of {entities.length} {entities.length === 1 ? 'entity' : 'entities'}</span>
                            </div>
                        )}
                        {!isLoading && filteredEntities.length === 0 && <div className="p-2 text-sm text-slate-500 dark:text-slate-400 text-center">No entities found</div>}
