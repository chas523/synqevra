...
const PROVIDER_TEMPLATES: Record<string, Partial<MailConfig>> = {
    GOOGLE: {
        ...
        enableTls: true,
        smtpProtocol: 'smtp',
        authUri: 'https://accounts.google.com/o/oauth2/v2/auth?prompt=consent&access_type=offline',
                    ...
                    <InfoTooltip
                        content={
                            <div className="space-y-2">
                                ...
                                <p>Select a provider for automatic configuration or use 'Custom' for manual setup.</p>
                            </div>
                        }
                    />
                </CardAction>
                ...
                <form id={formId} onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            {errors.mailFrom && <p className="text-xs text-destructive">{errors.mailFrom.message}</p>}
                        </div>
                            ...
                            <div className="space-y-4 pt-2">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input {...register("clientId", { required: enableOauth2 ? "Client ID is required" : false })} placeholder="OAuth2 Client ID" />
                                        {errors.clientId && <p className="text-xs text-destructive">{errors.clientId.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label>Client Secret *</Label>
                                        <Input type="password" {...register("clientSecret", { required: enableOauth2 ? "Client Secret is required" : false })} placeholder="OAuth2 Client Secret" />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input {...register("authUri", { required: enableOauth2 ? "Authorization URI is required" : false })} placeholder="https://..." />
                                        {errors.authUri && <p className="text-xs text-destructive">{errors.authUri.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label>Token URI *</Label>
                                        <Input {...register("tokenUri", { required: enableOauth2 ? "Token URI is required" : false })} placeholder="https://..." />
                                        {errors.tokenUri && <p className="text-xs text-destructive">{errors.tokenUri.message}</p>}
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <Label>Scope *</Label>
                                    <Input
                                        {...register("scope", {
                                            ...
                                            setValueAs: (v) => typeof v === 'string' ? v.split(' ').filter(s => s) : v
                                        })}
                                ...
                                <div className="p-4 border rounded-lg bg-muted/10 space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                {...register("redirectUriDomain", { required: enableOauth2 ? "Domain name is required" : false })}
                                                placeholder="localhost"
                                            />
                                            {errors.redirectUriDomain && <p className="text-xs text-destructive">{errors.redirectUriDomain.message}</p>}
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-xs text-muted-foreground">Redirect URI template</Label>
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <Button type="button" variant="ghost" size="sm" onClick={() => navigator.clipboard.writeText(redirectUriTemplate)}>
                                                Copy
                                            </Button>
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center justify-between pt-2 border-t">
                                    <span className="text-xs font-medium">Access token status: <span className="text-muted-foreground">not generated</span></span>
                        ...
                        {enableProxy && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                                <div className="space-y-2">
                                    ...
                                    <Input {...register("proxyHost", { required: enableProxy ? "Proxy Host is required" : false })} placeholder="proxy.example.com" />
                                    {errors.proxyHost && <p className="text-xs text-destructive">{errors.proxyHost.message}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label>Proxy Port *</Label>
                                    <Input type="number" {...register("proxyPort", { required: enableProxy ? "Proxy Port is required" : false, valueAsNumber: true })} placeholder="1080" />
                                    {errors.proxyPort && <p className="text-xs text-destructive">{errors.proxyPort.message}</p>}
                                </div>
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input type="password" {...register("proxyPassword")} placeholder="Password" />
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 pt-4 border-t">
                        <div className="flex gap-3 md:ml-auto">
                            <Button
                                type="button"
                                variant="outline"
                                onClick={() => reset(initialSettings?.jsonValue || DEFAULT_MAIL_CONFIG)}
                            >
                                Undo
                            </Button>
                            <LoadingButton
