...
import {
  RuleChainLatestTelemetryResponse,
  RuleChainService,
} from "@/lib/services/thingsboardServices/ruleChainService";

interface RuleChainLatestTelemetryTabContentProps {
  ruleChainId: string;
}
...
const mergeTelemetryResponse = (
  serverTelemetry: RuleChainLatestTelemetryResponse | undefined,
  optimisticTelemetry: RuleChainLatestTelemetryResponse,
  allowedKeys?: string[],
): RuleChainLatestTelemetryResponse => {
  const merged: RuleChainLatestTelemetryResponse = {
    ...(serverTelemetry || {}),
  };
...
export function RuleChainLatestTelemetryTabContent({
  ruleChainId,
}: RuleChainLatestTelemetryTabContentProps) {
  const { resolvedTheme } = useTheme();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [optimisticTelemetry, setOptimisticTelemetry] =
    useState<RuleChainLatestTelemetryResponse>({});
  ...
  const { data: attributes, isLoading: isLoadingAttributes } = useSWR(
    ...
    async () => RuleChainService.fetchRuleChainSharedAttributes(ruleChainId),
  );
  ...
  const {
    ...
    ruleChainId && configuredKeys.length > 0
  ...
  const {
    ...
    ruleChainId ? ["ruleChainLatestTelemetryKeys", ruleChainId] : null,
    async () => RuleChainService.fetchRuleChainLatestTelemetryKeys(ruleChainId),
  );
  ...
  const {
    ...
    ruleChainId && allTelemetryKeys && allTelemetryKeys.length > 0
      ? ["ruleChainLatestTelemetryAll", ruleChainId, allTelemetryKeys.join(",")]
      : null,
    async () =>
      RuleChainService.fetchRuleChainLatestTelemetry(
        ruleChainId,
  ...
  const handleSubmit = async () => {
    ...
    try {
      await RuleChainService.addRuleChainLatestTelemetry(ruleChainId, {
        [key]: parsedValue,
      });

      setOptimisticTelemetry((current: RuleChainLatestTelemetryResponse) => ({
        ...current,
        ...
        emptyMessage={
          searchQuery.trim()
            ...
            : configuredKeys.length === 0
              ? "No telemetry keys configured for this ruleChain."
          ...
          <AccordionContent className="pt-2">
            <DataTable
              ...
              emptyMessage={
                searchQuery.trim()
                  ? "No telemetry keys match your search."
                  : "No latest telemetry found for this ruleChain."
              }
