...
function DomainDetailsForm({
  ...
  const [propagateToEdge, setPropagateToEdge] = useState(
    domain.propagateToEdge,
  );
  ...
  >(
    Object.fromEntries(
      (domain.oauth2ClientInfos ?? []).map((c) => [c.id.id, c]),
    ),
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await OAuth2Service.updateDomain(
        domain.id.id,
        payload,
        selectedClientIds,
      );
      ...
      tabs={[
        {
          ...
          label: "Details",
          content: (
            <DomainDetailsForm domain={domain} onSaveSuccess={onSaveSuccess} />
          ),
        },
      ]}
    />
