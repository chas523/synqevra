...
// ─── OAuth2 Client Multi-Select (same pattern as AddDomainModal) ─────────────
    ...
    return (
        <div className="relative" ref={containerRef}>
            <div
                ...
                onClick={() => { setIsOpen(true); loadClients(); }}
            >
                {selectedIds.map((id) => {
                    const client = selectedCache[id] || clients.find((c) => c.id.id === id);
                    return (
                        <span key={id} className="flex items-center gap-1 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded-full text-xs dark:text-white">
                            {client?.title || id.substring(0, 8)}
                            <button type="button" className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 ml-0.5" onClick={(e) => removeId(id, e)}>×</button>
                        </span>
                    );
                })}
                <input
                    ...
                    onFocus={() => { setIsOpen(true); loadClients(); }}
                />
            </div>
            {isOpen && (
                <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-52 overflow-y-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg">
                    ...
                    {!isLoading && filtered.length === 0 && (
                        <div className="py-4 text-sm text-center text-slate-400 dark:text-slate-500">No clients found</div>
                    )}
                    {!isLoading && filtered.length > 0 && (
                        <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-700 text-xs font-semibold text-slate-500 dark:text-slate-400">
                            List of {filtered.length} {filtered.length === 1 ? "client" : "clients"}
                        </div>
                    )}
                    {filtered.map((client) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {isSelected && <span className="text-[#2a456c] dark:text-blue-400 font-medium">✓</span>}
                            </div>
                        );
                    })}
                </div>
...
function DomainDetailsForm({
    ...
    const [oauth2Enabled, setOauth2Enabled] = useState(domain.oauth2Enabled);
    const [propagateToEdge, setPropagateToEdge] = useState(domain.propagateToEdge);
    const [selectedClientIds, setSelectedClientIds] = useState<string[]>(
        domain.oauth2ClientInfos?.map((c) => c.id.id) ?? [],
    );
    const [selectedClientCache, setSelectedClientCache] = useState<Record<string, OAuth2ClientInfo>>(
        Object.fromEntries((domain.oauth2ClientInfos ?? []).map((c) => [c.id.id, c])),
    );
    const [isSaving, setIsSaving] = useState(false);

    // Update form when domain changes (different row selected)
    useEffect(() => {
        setName(domain.name);
        setOauth2Enabled(domain.oauth2Enabled);
        setPropagateToEdge(domain.propagateToEdge);
        setSelectedClientIds(domain.oauth2ClientInfos?.map((c) => c.id.id) ?? []);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await OAuth2Service.updateDomain(domain.id.id, payload, selectedClientIds);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Domain name*</label>
                <Input
                    value={name}
            ...
            <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Redirect URI template</label>
            ...
            <div className="flex items-center gap-3 py-1">
                ...
                />
                <span className="text-sm font-medium dark:text-white">Enable OAuth 2.0 settings</span>
            </div>

            {/* OAuth 2.0 clients */}
            <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400">OAuth 2.0 clients</label>
            ...
            <div className="flex items-center gap-3 py-1">
                <Switch
                    ...
                    disabled={isSaving}
                />
                <span className="text-sm font-medium dark:text-white">Propagate to Edge</span>
            </div>
...
export function DomainDetailPanel({ domain, onClose, onSaveSuccess }: DomainDetailPanelProps) {
