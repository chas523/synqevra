...
export function OAuth2ClientDetailPanel({ client, onClose, onSaveSuccess }: OAuth2ClientDetailPanelProps) {
    ...
    const handleSave = async (payload: OAuth2ClientPayload) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to update OAuth 2.0 client");
        } finally {
            setIsSaving(false);
        }
    };
