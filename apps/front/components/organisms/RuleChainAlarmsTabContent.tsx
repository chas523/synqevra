...
interface RuleChainAlarmsTabContentProps {
  ruleChainId: string;
}

interface RuleChainAlarm {
  id: { id: string };
  createdTime: number;
...
interface RuleChainAlarmsResponse {
  data: RuleChainAlarm[];
  totalPages: number;
...
export function RuleChainAlarmsTabContent({
  ruleChainId,
}: RuleChainAlarmsTabContentProps) {
  ...
  const { data, isLoading, mutate } = useSWR<RuleChainAlarmsResponse>(
    ruleChainId
      ? [
          "ruleChainAlarms",
          ruleChainId,
      ...
      : null,
    async () => {
      return RuleChainService.getRuleChainAlarms(
        ruleChainId,
        page,
        pageSize,
        statusList,
  ...
  const columns: DataTableColumn<RuleChainAlarm>[] = useMemo(
    () => [
      {
        key: "createdTime",
        header: "Created time",
  ...
  const filteredAlarms = useMemo(() => {
    ...
    return (data?.data || []).filter((alarm: RuleChainAlarm) => {
      ...
      <DataTable
        ...
        emptyMessage={
          isSearching
            ...
            : "No alarms found for this ruleChain."
        }
