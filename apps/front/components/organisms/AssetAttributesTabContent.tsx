...
export function AssetAttributesTabContent({ assetId }: AssetAttributesTabContentProps) {
    ...
    const { data: attributes, isLoading, mutate } = useSWR(
    ...
    const handleAddAttribute = async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : "Invalid attribute value");
            return;
        }
        ...
        try {
            ...
            toast.error(error?.response?.data?.message || "Failed to add server attribute");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const dataArray: AttributeRow[] = useMemo(() => {
        ...
        return Object.entries(attributes as Record<string, unknown>).map(([key, value], index) => ({
            id: `${key}-${index}`,
            key,
                    ...
                    <div className="space-y-4 py-2">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-[160px_1fr]">
                            <div className="space-y-1.5">
                                ...
                                <SelectAdmin
                                    value={attributeType}
                                    onValueChange={(value) => setAttributeType(value as AttributeType)}
                                    disabled={isSaving}
                                >
