...
interface DeviceProfileDetailsTabContentProps {
  profileId: string;
}
...
export function DeviceProfileDetailsTabContent({
  profileId,
}: DeviceProfileDetailsTabContentProps) {
  const {
    data: profile,
    isLoading,
    mutate,
  } = useSWR(profileId ? ["deviceProfileDetails", profileId] : null, async () =>
  ...
  const { data: defaultQueue, isLoading: isLoadingDefaultQueue } = useSWR(
    !isEditing && defaultQueueName
      ? ["deviceProfileDefaultQueueByName", defaultQueueName]
      : null,
  ...
  const { data: defaultRuleChain, isLoading: isLoadingDefaultRuleChain } =
    useSWR(
      !isEditing && defaultRuleChainId
        ? ["deviceProfileDefaultRuleChainById", defaultRuleChainId]
        : null,
  ...
  const {
    ...
    !isEditing && defaultEdgeRuleChainId
      ? ["deviceProfileDefaultEdgeRuleChainById", defaultEdgeRuleChainId]
      : null,
      ...
      return (
        <div className="grid gap-3 md:grid-cols-[104px_minmax(0,1fr)_auto] md:items-end">
          <div className="flex h-26.5 items-center justify-center overflow-hidden rounded-md border bg-muted/20">
            {currentForm.image ? (
              <ImagePreview src={currentForm.image} alt="Device profile" />
            ) : (
          ...
          <div className="space-y-2">
            <label
              htmlFor="device-profile-details-image-link"
              className="text-sm font-medium"
            >
              Image link
            </label>
            <Input
              id="device-profile-details-image-link"
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (refreshed) {
        const nextForm = toFormState(refreshed);
        setForm(nextForm);
        setOriginalForm(nextForm);
      }
  ...
  if (!profile || !form) {
    return (
      <div className="p-4 text-center text-slate-500">
        Device profile details not found.
      </div>
    );
  }
