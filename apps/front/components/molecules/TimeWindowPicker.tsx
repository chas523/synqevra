...
const RELATIVE_OPTIONS: RelativeOption[] = [
  ...
  {
    key: "prev_week_mon_sun",
    label: "Previous week (Mon - Sun)",
    getRange: () => {
      const d = new Date();
  ...
  {
    key: "current_day_so_far",
    label: "Current day so far",
    getRange: () => ({ start: startOfDay(now()), end: now() }),
  },
  {
    key: "current_week_sun_sat",
    label: "Current week (Sun - Sat)",
    getRange: () => {
      const d = now();
      const sun = new Date(d);
  ...
  {
    key: "current_week_mon_sun",
    label: "Current week (Mon - Sun)",
    getRange: () => {
      const d = now();
      const mon = new Date(d);
  ...
  {
    key: "current_week_so_far_sun",
    label: "Current week so far (Sun - Sat)",
    getRange: () => {
      const d = now();
      const sun = new Date(d);
      sun.setDate(d.getDate() - d.getDay());
  ...
  {
    key: "current_week_so_far_mon",
    label: "Current week so far (Mon - Sun)",
    getRange: () => {
      const d = now();
      const mon = new Date(d);
      mon.setDate(d.getDate() - ((d.getDay() + 6) % 7));
  ...
  {
    key: "current_quarter_so_far",
    label: "Current quarter so far",
    getRange: () => {
      const q = Math.floor(now().getMonth() / 3);
  ...
  },
  {
    ...
    getRange: () => {
      ...
      return {
        ...
        end: new Date(now().getFullYear(), h * 6 + 6, 0),
      };
    },
  },
  {
    key: "current_half_so_far",
    label: "Current half year so far",
    getRange: () => {
      const h = now().getMonth() < 6 ? 0 : 1;
...
export function TimeWindowPicker({ value, onChange }: TimeWindowPickerProps) {
  ...
  const [draftLastLabel, setDraftLastLabel] = useState(
    value.lastLabel ?? "1 day",
  );
  const [draftRelativeKey, setDraftRelativeKey] = useState(
    value.relativeKey ?? "current_day",
  );
  ...
  const handleUpdate = () => {
    if (draftMode === "last") {
      ...
      onChange({
        mode: "relative",
        relativeKey: draftRelativeKey,
        relativeLabel: opt.label,
        startTime: r.start,
        endTime: r.end,
      });
    } else {
