        ...
        <div className="w-full h-full bg-white dark:bg-slate-950 flex flex-col">
            <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full">
                <Tabs.List className="flex border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-slate-900">
                    <Tabs.Trigger
                        value="resources"
                        className={`px-4 py-2 text-sm font-medium outline-none ${activeTab === 'resources'
                    ...
                    <Tabs.Trigger
                        value="html"
                        className={`px-4 py-2 text-sm font-medium outline-none ${activeTab === 'html'
                            ? 'text-blue-600 border-b-2 border-blue-600 bg-white dark:bg-slate-950 dark:text-blue-400'
                            : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'
                            }`}
                    >
                    ...
                    <Tabs.Trigger
                        value="css"
                        className={`px-4 py-2 text-sm font-medium outline-none ${activeTab === 'css'
                            ? 'text-blue-600 border-b-2 border-blue-600 bg-white dark:bg-slate-950 dark:text-blue-400'
                            : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'
                            }`}
                    >
...
function ResourcesTab() {
    ...
    const removeResource = (index: number) => {
        const newResources = existingResources.filter((_: any, i: number) => i !== index);
        if (widgetType && widgetType.descriptor) {
            updateDescriptor({
                ...widgetType.descriptor,
                resources: newResources
    ...
    const options = filteredResources.map((item) => (
        <Combobox.Option value={item.link || ''} key={item.id?.id}>
            <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{item.link}</span>
            </div>
        </Combobox.Option>
    ));
                    ...
                    >
                        <Combobox.Target>
                            <TextInput
                                ...
                                rightSection={
                                    searchTerm && <X size={16} className="cursor-pointer text-gray-400" onClick={() => setSearchTerm('')} />
                                }
                                classNames={{
                        ...
                        <Combobox.Dropdown className="bg-white dark:bg-slate-900 border-gray-200 dark:border-gray-800">
                            <Combobox.Options className="overflow-y-auto">
                                {options.length > 0 ? options : <Combobox.Empty className="text-gray-500 dark:text-gray-400">No results found</Combobox.Empty>}
                            </Combobox.Options>
                        </Combobox.Dropdown>
                    </Combobox>
                </div>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        {existingResources.map((res: any, index: number) => (
                            <tr key={index} className="hover:bg-gray-50 dark:hover:bg-slate-900 text-gray-900 dark:text-gray-100">
