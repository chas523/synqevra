...
export function SettingsSchemaTable({ schemaJson, onChange }: SettingsSchemaTableProps) {
    const [properties, setProperties] = useState<PropertyDefinition[]>([]);
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [editingProperty, setEditingProperty] = useState<PropertyDefinition | undefined>(undefined);
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-1 overflow-auto border border-gray-200 dark:border-gray-800 rounded-md">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        {properties.map((prop) => (
                            <tr key={prop.id} className="hover:bg-gray-50 dark:hover:bg-slate-900 text-gray-900 dark:text-gray-100">
                                <td className="px-4 py-2">{prop.id}</td>
                                <td className="px-4 py-2">{prop.name}</td>
                                <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{prop.type}</td>
            ...
            <div className="pt-4">
                <Button onClick={openAddDialog} className="bg-white dark:bg-slate-950 border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-900">
                    Add property
                </Button>
            </div>
