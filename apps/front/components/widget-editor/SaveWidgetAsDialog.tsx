...
export const SaveWidgetAsDialog: React.FC<SaveWidgetAsDialogProps> = ({
  ...
  const fetchBundles = async () => {
    try {
      const data = await WidgetService.getWidgetsBundles(
        0,
        100,
        "title",
        "ASC",
  ...
  const handleSaveAs = async () => {
    ...
    try {
      ...
      const currentFqns =
        await WidgetService.getWidgetTypeFqns(selectedBundleId);
      ...
      if (newWidgetIdentifier) {
        ...
        console.warn(
          "Could not determine Identifier (alias/fqn) for new widget, skipping bundle link update.",
        );
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a copy of this widget. Choose a name and a destination
            bundle.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button
            ...
            disabled={loading || !widgetName || !selectedBundleId}
          >
            {loading ? "Saving..." : "Save"}
          </Button>
        </DialogFooter>
