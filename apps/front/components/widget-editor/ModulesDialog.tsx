...
export function ModulesDialog({ open, onOpenChange, modules, onChange }: ModulesDialogProps) {
    ...
    const handleUpdateModule = (index: number, field: keyof ModuleItem, value: string) => {
                ...
                <div className="flex-1 overflow-y-auto py-4 space-y-4">
                    ...
                    {localModules.map((module, index) => (
                        <div key={index} className="grid grid-cols-[1fr_2fr_auto] gap-4 items-start">
                            <Input
                                value={module.alias}
                                onChange={(e) => handleUpdateModule(index, 'alias', e.target.value)}
                                placeholder="Set"
                            ...
                            >
                                <Combobox.Target>
                                    <TextInput
                                        ...
                                        value={module.url}
                                        onChange={(event) => {
                                            handleUpdateModule(index, 'url', event.currentTarget.value);
                                            ...
                                            filteredResources.map(res => (
                                                <Combobox.Option value={res.link || ''} key={res.id?.id}>
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-xs text-gray-500">{res.link}</span>
                                                    </div>
                                                </Combobox.Option>
                                            ))
                                        ) : (
                                            <div className="p-2 text-sm text-gray-500">No JS modules found</div>
                                        )}
                                    </Combobox.Options>
                                </Combobox.Dropdown>
                            </Combobox>
