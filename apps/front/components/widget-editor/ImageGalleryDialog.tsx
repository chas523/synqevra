...
import { LayoutGrid, List as ListIcon, Upload, Search, RefreshCw, FileImage, X } from 'lucide-react';
...
export function ImageGalleryDialog({ open, onOpenChange, onSelect }: ImageGalleryDialogProps) {
    ...
    const fetchImages = async (pageParam = 0) => {
        ...
        try {
            const pageSize = 36;
            const response = await ResourceService.getResources(pageParam, pageSize, 'createdTime', 'DESC', 'IMAGE', 'IMAGE');
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[900px] h-[80vh] flex flex-col p-0 gap-0 bg-white dark:bg-slate-950">
                <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
                    <DialogTitle className="text-lg font-semibold">Image gallery</DialogTitle>
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" size="icon" onClick={() => fetchImages(0)} title="Refresh">
                            <RefreshCw size={18} />
                        </Button>
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-slate-900/50">
                            ...
                            {images.map((img, index) => (
                                ...
                                >
                                    ...
                                    <div className="ml-3 flex-1 min-w-0">
                                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{img.title}</div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{img.resourceKey}</div>
                                    </div>
                                    <div className="text-xs text-gray-400">
                                        {img.createdTime ? new Date(img.createdTime).toLocaleDateString() : '-'}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {loading && (
                        <div className="py-4 text-center text-gray-500">Loading images...</div>
                    )}
                ...
                <DialogFooter className="p-4 border-t border-gray-200 dark:border-gray-800">
                    ...
                    <Button onClick={handleSelect} disabled={!selectedImage} className="bg-blue-600 hover:bg-blue-700 text-white">Embed</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
