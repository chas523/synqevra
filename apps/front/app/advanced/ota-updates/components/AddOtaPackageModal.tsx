...
import { SelectAdmin as Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/admin_select";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import { FileDropzone } from "@/components/molecules/FileDropzone";
import { CreateOtaPackageRequest, ChecksumAlgorithm, CHECKSUM_ALGORITHM_OPTIONS } from "@/types/otaPackageTypes";
...
export function AddOtaPackageModal({ isOpen, onClose, onAdd, isSaving }: AddOtaPackageModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload: CreateOtaPackageRequest = {
            ...
            type,
            deviceProfileId: { entityType: "DEVICE_PROFILE", id: selectedDeviceProfileId },
            isURL,
        };
    ...
    const isFormValid = title && version && selectedDeviceProfileId && (uploadType === "upload" || url);
        ...
        <Dialog open={isOpen} onOpenChange={(open) => { if (!open) { onClose(); resetForm(); } }}>
            <DialogContent className="sm:max-w-[550px] overflow-y-auto max-h-[90vh]">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <Input
                                ...
                                className="dark:text-white"
                            />
                            <p className="text-xs text-muted-foreground">Custom tag should match the package version reported by your device.</p>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                <SelectTrigger className="w-full dark:text-white">
                                    <SelectValue placeholder={isLoadingProfiles ? "Loading profiles..." : "Select device profile"} />
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            <p className="text-xs text-muted-foreground">The uploaded package will be available only for devices with the chosen profile.</p>
                        </div>

                        {/* Package type */}
                        <div className="space-y-1">
                            <Label className="dark:text-white">Package type*</Label>
                            <Select value={type} onValueChange={(val: string) => setType(val as "FIRMWARE" | "SOFTWARE")}>
                                <SelectTrigger className="w-full dark:text-white">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            <p className="text-xs text-muted-foreground">Once the package is uploaded, you will not be able to modify title, version, device profile and package type.</p>
                        </div>

                        {/* Upload type radio */}
                        <RadioGroup
                            value={uploadType}
                            onValueChange={(val: string) => setUploadType(val as "upload" | "url")}
                            className="flex items-center gap-6"
                        >
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="upload" id="upload-binary" />
                                <Label htmlFor="upload-binary" className="cursor-pointer dark:text-white">Upload binary file</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="url" id="use-url" />
                                <Label htmlFor="use-url" className="cursor-pointer dark:text-white">Use external URL</Label>
                            </div>
                        </RadioGroup>
                        ...
                        {uploadType === "url" ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label className="dark:text-white">Checksum algorithm</Label>
                                        <Select value={checksumAlgorithm} onValueChange={(val: string) => setChecksumAlgorithm(val as ChecksumAlgorithm)}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className="dark:text-white"
                                        />
                                        <p className="text-xs text-muted-foreground">If checksum is empty, it will be generated automatically</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <DialogFooter>
                        <Button type="button" variant="outline" onClick={() => { onClose(); resetForm(); }} disabled={isSaving}>
                            Cancel
                        </Button>
