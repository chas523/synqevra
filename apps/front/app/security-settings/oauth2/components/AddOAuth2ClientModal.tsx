...
export function AddOAuth2ClientModal({ open, onOpenChange, onSuccess }: AddOAuth2ClientModalProps) {
    ...
    const handleSave = async (payload: OAuth2ClientPayload) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to add OAuth 2.0 client");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                ...
                <div className="h-[650px] overflow-hidden flex flex-col">
