...
function OAuth2ClientChips({ clients }: { clients: OAuth2ClientInfo[] }) {
    if (!clients || clients.length === 0) return <span className="text-slate-400 dark:text-slate-500 text-sm">—</span>;
    return (
...
export default function DomainsPage() {
    ...
    const { domains, totalPages, totalElements, isLoading, mutate } = useDomainInfos(
        currentPage,
        PAGE_SIZE,
        sortProperty,
        sortOrder,
