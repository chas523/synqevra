...
// Fields per provider
const ANTHROPIC_FIELDS = ["temperature", "topP", "topK", "maxOutputTokens"] as const;
const OPENAI_MISTRAL_FIELDS = ["temperature", "topP", "presencePenalty", "frequencyPenalty", "maxOutputTokens"] as const;
...
function AiModelModal({ open, onClose, onSaved, editModel }: AiModelModalProps) {
    const [name, setName] = useState("");
    const [provider, setProvider] = useState<ProviderType>("ANTHROPIC");
    const [apiKey, setApiKey] = useState("");
    const [showApiKey, setShowApiKey] = useState(false);
    const [modelId, setModelId] = useState("");
    const [configFields, setConfigFields] = useState<Record<string, number | null>>({});
    ...
    useEffect(() => {
        if (editModel) {
            ...
            for (const key of ["temperature", "topP", "topK", "maxOutputTokens", "presencePenalty", "frequencyPenalty"]) {
    ...
    const handleCheckConnectivity = useCallback(async () => {
        ...
        try {
            ...
            setConnectivityResult({ status: "FAILURE", errorDetails: error?.message || "Connection failed" });
        } finally {
            setIsChecking(false);
        }
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            if (provider === "ANTHROPIC") {
                ...
                payload.configuration.frequencyPenalty = configFields.frequencyPenalty ?? null;
                payload.configuration.presencePenalty = configFields.presencePenalty ?? null;
            }
    ...
    }, [name, provider, apiKey, modelId, configFields, editModel, onSaved, onClose]);

    const fields = getFieldsForProvider(provider);
    const canCheck = name.trim() !== "" && apiKey.trim() !== "" && modelId.trim() !== "";
    ...
    return (
        <>
            <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto dark:bg-slate-900 dark:border-slate-700">
                    <DialogHeader className="bg-white dark:bg-slate-800 -m-6 mb-0 p-4 rounded-t-lg">
                        ...
                        <DialogDescription className="sr-only">Configure an AI model</DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-5 pt-4">
                        ...
                        <fieldset className="border border-slate-300 dark:border-slate-600 rounded-md p-4 space-y-4">
                            <legend className="text-sm font-semibold text-foreground dark:text-white px-2">Provider</legend>

                            <div className="space-y-1">
                                <Label className="text-xs text-muted-foreground dark:text-slate-400">AI provider</Label>
                                <Select value={provider} onValueChange={(val: string) => setProvider(val as ProviderType)}>
                        ...
                        <fieldset className="border border-slate-300 dark:border-slate-600 rounded-md p-4 space-y-3">
                            <legend className="text-sm font-semibold text-foreground dark:text-white px-2">Configuration</legend>
                            ...
                            {fields.map((field) => (
                                <fieldset key={field} className="border border-slate-200 dark:border-slate-700 rounded-md p-3">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className="text-sm text-foreground dark:text-white">{FIELD_LABELS[field]}</span>
            ...
            <Dialog open={connectivityResult !== null} onOpenChange={() => setConnectivityResult(null)}>
                <DialogContent className="sm:max-w-md dark:bg-slate-900 dark:border-slate-700">
                    <DialogHeader>
                        <DialogTitle className="dark:text-white">
                            {connectivityResult?.status === "SUCCESS" ? "✅ Connection successful" : "❌ Connection failed"}
                        </DialogTitle>
                        <DialogDescription className="sr-only">Connectivity check result</DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
...
export function AiModelsTab() {
    ...
    const fetchModels = useCallback(async () => {
        setIsLoading(true);
        try {
            const result = await SettingsService.getAiModels(currentPage, pageSize, sortProperty, sortOrder);
            setData(result.data || []);
    ...
    const handleSortChange = useCallback((property: string) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: false,
            render: (item: any) => {
                const prov = PROVIDERS.find((p) => p.value === item.configuration?.provider);
                return <span className="text-sm text-foreground dark:text-white">{prov?.label || item.configuration?.provider}</span>;
            },
        },
        {
            key: "modelId",
            ...
            sortable: false,
            render: (item: any) => (
                <span className="text-sm text-primary dark:text-blue-400">{item.configuration?.modelId}</span>
            ),
        },
    ];
