...
export default function NotificationsLayout({
    ...
    const tabs = [
        ...
        { name: "Recipients", path: "/dashboard/notifications/recipients", icon: Users },
        { name: "Templates", path: "/dashboard/notifications/templates", icon: Users }, // Using Users icon as placeholder, or FileText if available
                ...
                <nav className="flex gap-4" aria-label="Tabs">
                    {tabs.map((tab) => {
                        const isActive = pathname === tab.path ||
                            (pathname === '/dashboard/notifications' && tab.path.includes('inbox'));
                        const Icon = tab.icon;
