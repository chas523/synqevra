      ...
      try {
        ...
        const providersMap: Record<string, ProviderConfig> = {
          ...
          SMS: {
            providerType: "SMS",
            enabled: false,
            verificationMessageTemplate: "Verification code: ${code}",
        ...
        reset({
          ...
          minVerificationCodeSendPeriod:
            data.minVerificationCodeSendPeriod || 30,
          maxVerificationFailuresBeforeUserLockout:
            data.maxVerificationFailuresBeforeUserLockout || 30,
          totalAllowedTimeForVerification:
            data.totalAllowedTimeForVerification || 3600,
          rateLimitEnabled,
          rateLimitAttempts,
          rateLimitTime,
        });
  ...
  const onSubmit = async (values: SettingsFormValues) => {
    ...
    try {
      ...
      const payload: TwoFactorAuthSettings = {
        ...
        maxVerificationFailuresBeforeUserLockout:
          values.maxVerificationFailuresBeforeUserLockout,
            ...
            <div className="space-y-4">
              ...
              {watchedProviders?.[0]?.enabled && (
                <div className="pl-0 pt-2 animate-in fade-in slide-in-from-top-2 duration-200 space-y-2">
                  ...
                  <Input
                    {...register("providers.0.issuerName")}
                    className="bg-muted/30"
                  />
                  {errors.providers?.[0]?.issuerName && (
                    <p className="text-xs text-destructive">
                      {errors.providers[0].issuerName?.message}
                    </p>
                  )}
                </div>
              )}
              ...
              {watchedProviders?.[1]?.enabled && (
                <div className="grid grid-cols-2 gap-4 pt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="space-y-2">
                    ...
                    <Input
                      {...register("providers.1.verificationMessageTemplate")}
                      className="bg-muted/30"
                    />
                    {errors.providers?.[1]?.verificationMessageTemplate && (
                      <p className="text-xs text-destructive">
                        {
                          errors.providers[1].verificationMessageTemplate
                            ?.message
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      type="number"
                      {...register("providers.1.verificationCodeLifetime")}
                      className="bg-muted/30"
                    />
                    {errors.providers?.[1]?.verificationCodeLifetime && (
                      <p className="text-xs text-destructive">
                        {errors.providers[1].verificationCodeLifetime?.message}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              {watchedProviders?.[2]?.enabled && (
                <div className="pt-2 animate-in fade-in slide-in-from-top-2 duration-200 space-y-2">
                  ...
                  <Input
                    type="number"
                    {...register("providers.2.verificationCodeLifetime")}
                    className="bg-muted/30"
                  />
                  {errors.providers?.[2]?.verificationCodeLifetime && (
                    <p className="text-xs text-destructive">
                      {errors.providers[2].verificationCodeLifetime?.message}
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              {watchedProviders?.[3]?.enabled && (
                <div className="pt-2 animate-in fade-in slide-in-from-top-2 duration-200 space-y-2">
                  ...
                  <Input
                    type="number"
                    {...register("providers.3.backupCodeCount")}
                    className="bg-muted/30"
                  />
                  {errors.providers?.[3]?.backupCodeCount && (
                    <p className="text-xs text-destructive">
                      {errors.providers[3].backupCodeCount?.message}
                    </p>
                  )}
                </div>
              )}
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  {...register("totalAllowedTimeForVerification")}
                  className="bg-muted/30"
                />
                {errors.totalAllowedTimeForVerification && (
                  <p className="text-xs text-destructive">
                    {errors.totalAllowedTimeForVerification.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">
                  Retry verification code period (sec)*
                </Label>
                <Input
                  type="number"
                  {...register("minVerificationCodeSendPeriod")}
                  className="bg-muted/30"
                />
                {errors.minVerificationCodeSendPeriod && (
                  <p className="text-xs text-destructive">
                    {errors.minVerificationCodeSendPeriod.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  {...register("maxVerificationFailuresBeforeUserLockout")}
                  className="bg-muted/30"
                />
                {errors.maxVerificationFailuresBeforeUserLockout && (
                  <p className="text-xs text-destructive">
                    {errors.maxVerificationFailuresBeforeUserLockout.message}
                  </p>
                )}
              </div>
            </div>
            ...
            {rateLimitEnabled && (
              <div className="grid grid-cols-2 gap-6 pt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">
                    Number of checking attempts*
                  </Label>
                  <Input
                    {...register("rateLimitAttempts")}
                    className="bg-muted/30"
                  />
                  {errors.rateLimitAttempts && (
                    <p className="text-xs text-destructive">
                      {errors.rateLimitAttempts.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    {...register("rateLimitTime")}
                    className="bg-muted/30"
                  />
                  {errors.rateLimitTime && (
                    <p className="text-xs text-destructive">
                      {errors.rateLimitTime.message}
                    </p>
                  )}
                </div>
              </div>
