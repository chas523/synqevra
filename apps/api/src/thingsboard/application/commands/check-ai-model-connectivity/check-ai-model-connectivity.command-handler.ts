...
export class CheckAiModelConnectivityCommandHandler implements ICommandHandler<
  CheckAiModelConnectivityCommand,
...
> {
  ...
  async execute(
    command: CheckAiModelConnectivityCommand,
  ): Promise<Result<any, ThingsboardApiException>> {
    const { accessToken, payload } = command;
    try {
      const response = await this.thingsboardApi.checkAiModelConnectivity(
        accessToken,
        payload,
      );
      return Ok(response);
