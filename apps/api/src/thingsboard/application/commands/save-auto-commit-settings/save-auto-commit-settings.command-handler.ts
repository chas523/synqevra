...
export class SaveAutoCommitSettingsCommandHandler implements ICommandHandler<
  SaveAutoCommitSettingsCommand,
...
> {
  ...
  async execute(
    command: SaveAutoCommitSettingsCommand,
  ): Promise<Result<any, ThingsboardApiException>> {
    const { accessToken, payload } = command;
    try {
      const response = await this.thingsboardApi.saveAutoCommitSettings(
        accessToken,
        payload,
      );
      return Ok(response);
