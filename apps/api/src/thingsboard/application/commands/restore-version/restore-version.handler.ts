...
export class RestoreVersionHandler implements ICommandHandler<RestoreVersionCommand, Result<string, ThingsboardApiException>> {
    ...
    async execute(command: RestoreVersionCommand): Promise<Result<string, ThingsboardApiException>> {
        this.logger.debug(`Executing RestoreVersionCommand`);
        try {
            const response = await this.thingsboardApiPort.restoreVersion(command.accessToken, command.payload);
            return Ok(response);
        } catch (error) {
