...
export class SaveEntityAttributesCommandHandler implements ICommandHandler<
  SaveEntityAttributesCommand,
...
> {
  ...
  async execute(
    ...
    try {
      ...
      this.logger.log(
        `Successfully saved attributes for ${entityType}/${entityId}`,
      );
      return Ok(undefined);
    } catch (error) {
