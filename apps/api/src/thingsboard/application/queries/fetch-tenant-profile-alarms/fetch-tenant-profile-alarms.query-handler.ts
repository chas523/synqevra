...
export class FetchTenantProfileAlarmsQueryHandler
    implements
    IQueryHandler<
        FetchTenantProfileAlarmsQuery,
        Result<EntityAlarmsResponse, TBAdminGetError>
    > {
    private readonly logger = new Logger(FetchTenantProfileAlarmsQueryHandler.name);
    ...
    async execute(
        ...
        const { tenantProfileId, page, pageSize, statusList, severityList, startTime, endTime, accessToken } = query;
