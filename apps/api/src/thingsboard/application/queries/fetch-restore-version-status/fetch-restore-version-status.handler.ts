...
export class FetchRestoreVersionStatusHandler implements IQueryHandler<
  FetchRestoreVersionStatusQuery,
...
> {
  ...
  async execute(
    query: FetchRestoreVersionStatusQuery,
  ): Promise<Result<any, ThingsboardApiException>> {
    this.logger.debug(
      `Executing FetchRestoreVersionStatusQuery  for requestId: ${query.requestId}`,
    );
    try {
      const response = await this.thingsboardApiPort.getRestoreVersionStatus(
        query.accessToken,
        query.requestId,
      );
      return Ok(response);
