...
export class FetchDomainInfosQueryHandler implements IQueryHandler<
  FetchDomainInfosQuery,
...
> {
  ...
  async execute(
    query: FetchDomainInfosQuery,
  ): Promise<Result<any, ThingsboardApiException>> {
    try {
      const data = await this.thingsboardApi.getDomainInfos(
        query.accessToken,
        query.params,
      );
      return Ok(data);
