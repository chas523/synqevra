...
export class CreateMedplumTenantUseCase {
    ...
    async execute(dto: CreateMedplumRequestDto, uow: UnitOfWork): Promise<void> {
        this.logger.debug(`[execute] Looking up connection for tenantId=${dto.tenantId}`);
        ...
        this.logger.debug(`[execute] connection result: ${JSON.stringify(connection)}`);

        if (!connection) {
            this.logger.error(`[execute] No connection found for tenantId=${dto.tenantId}`);
            throw new NotFoundException(
        ...
        this.logger.debug(`[execute] Looking up thingsboard record for tenantId=${dto.tenantId}`);
        const thingsboard = await uow.thingsboardRepository.findByTenantId(
            dto.tenantId,
        );

        this.logger.debug(`[execute] thingsboard result: ${JSON.stringify(thingsboard)}`);
