...
export class PatientLoginUseCase {
    ...
    async execute(command: PatientLoginCommand): Promise<LoginResult> {
        ...
        const accessToken = await this.authService.generateAccessToken(patientId, Role.PATIENT);
        const refreshToken = await this.authService.generateRefreshToken(patientId, Role.PATIENT);
