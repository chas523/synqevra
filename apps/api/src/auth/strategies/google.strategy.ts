...
import { Strategy as GoogleOAuth20Strategy, VerifyCallback } from "passport-google-oauth20";
...
import { THINGSBOARD_API_PORT, ThingsboardApiPort } from "../../thingsboard/application/ports/thingsboard.api.port";
...
export class GoogleStrategy extends PassportStrategy(CustomStrategy, 'google') {
    ...
    async authenticate(req: Request, options?: any) {
        try {
            ...
            if (!googleConfig) {
                this.logger.error("No Google OAuth2 configuration found in ThingsBoard");
    ...
    private async getGoogleConfigFromThingsBoard() {
        ...
        const clientInfos = await this.thingsboardApi.getOAuth2ClientInfos(token, {
            page: 0, pageSize: 100, sortProperty: 'createdTime', sortOrder: 'DESC'
        });

        const googleSummary = (clientInfos?.data || []).find(
        ...
        const fullDetails = await this.thingsboardApi.getOAuth2ClientById(token, googleSummary.id.id);
     ...
     */
    private resolveInnerStrategy(config: { clientId: string, clientSecret: string }): GoogleOAuth20Strategy {
            ...
            this.innerStrategy = new GoogleOAuth20Strategy(
                {
                    ...
                    callbackURL: process.env.GOOGLE_OAUTH_CALLBACK_URL || `${process.env.BACKEND_API_URL}/auth/google/callback`,
                    scope: ['email', 'profile'],
                },
