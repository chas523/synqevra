...
export class SysAdminAuthService {
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (
      ...
      currentTime < this.tokenExpirationTime - this.TOKEN_REFRESH_BUFFER_SECONDS
    ) {
      return this.token;
    }
    ...
    try {
      ...
      if (payloadServices) {
        const payload = JSON.parse(
          Buffer.from(payloadServices, 'base64').toString(),
        );
        if (payload.exp) {
          this.tokenExpirationTime = payload.exp;
          this.logger.debug(
            `Token cached, expires at ${new Date(payload.exp * 1000).toISOString()}`,
          );
        } else {
      ...
      this.logger.warn(
        'Failed to parse JWT token expiration, using default timeout',
      );
