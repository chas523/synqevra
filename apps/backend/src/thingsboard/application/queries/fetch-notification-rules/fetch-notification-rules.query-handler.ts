...
export class FetchNotificationRulesQueryHandler implements IQueryHandler<
  FetchNotificationRulesQuery,
  Result<NotificationRulesResponse, ThingsboardApiException>
> {
  private readonly logger = new Logger(FetchNotificationRulesQueryHandler.name);
  ...
  async execute(
    ...
    try {
      ...
      this.logger.error(`Failed to fetch notification rules: ${error.message}`);
      return Err(
