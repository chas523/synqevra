...
export function TimeRangeFilter({ value, onChange }: TimeRangeFilterProps) {
  ...
  const getDisplayLabel = () => {
    ...
    if (value.type === "LAST")
      return `Last ${value.lastValue} ${value.lastUnit?.toLowerCase()}`;
    if (value.type === "RANGE") {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-106.25 dark:bg-slate-900 dark:border-slate-800">
        ...
        <Tabs
          ...
          onValueChange={(v) =>
            setTempValue((prev) => ({ ...prev, type: v as any }))
          }
          className="w-full"
        >
          <TabsList className="grid w-full grid-cols-3 dark:bg-slate-800">
            <TabsTrigger
              value="ALL_TIME"
              className="dark:data-[state=active]:bg-slate-700 dark:data-[state=active]:text-slate-100 dark:text-slate-400"
            >
              For all time
            </TabsTrigger>
            <TabsTrigger
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setTempValue((prev) => ({
                      ...prev,
                      lastValue: parseInt(e.target.value) || 1,
                    }))
                  }
              ...
              <div className="flex-1">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setTempValue((prev) => ({ ...prev, lastUnit: v as any }))
                  }
                />
              </div>
            </div>
        ...
        <DialogFooter>
          <Button
            ...
            className="dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100"
          >
            Cancel
          </Button>
