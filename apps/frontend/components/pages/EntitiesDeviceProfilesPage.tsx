  ...
  const handleImport = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        await DeviceService.createDeviceProfile(json);
        toast.success(`Device profile "${json.name ?? file.name}" imported`);
        mutate();
      } catch (error: any) {
        const message =
          error?.response?.data?.message ??
          "Failed to import device profile – invalid JSON file";
        toast.error(message);
      } finally {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label="Import device profile"
            >
              <Upload className="h-4 w-4" />
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>Import device profile from JSON</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      <input
