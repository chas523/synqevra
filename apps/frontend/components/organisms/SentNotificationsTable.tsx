...
export const SentNotificationsTable = ({
      ...
      render: (request) => {
        ...
        if (request.stats?.sent) {
          if (typeof request.stats.sent === "object") {
            sent = Object.values(
              request.stats.sent as Record<string, number>,
        ...
        if (request.stats?.errors) {
          if (typeof request.stats.errors === "object") {
            errors = Object.values(
              request.stats.errors as Record<string, number>,
            ).reduce((sum, val) => sum + val, 0);
          } else {
  ...
  return (
    <DataTable
      ...
      getRowId={(request) => {
        // Safely extract ID
        if (!request.id || typeof request.id !== "object")
